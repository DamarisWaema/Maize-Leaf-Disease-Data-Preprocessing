...
seq = iaa.Sequential([
    iaa.imgcorruptlike.SpeckleNoise(severity=2)], # Change here
...
def save_augmentations(images: list, bbs: list, df: pd.DataFrame, filename: str, folder: str, resize: bool = False,
    ...
    for [i, img_a], bb_a in zip(enumerate(images), bbs):
        ...
        aug_img_name = name[0]+'-SpeckleNoise.jpg' #change here
...
if __name__ == '__main__':
    ...
    output_folder = os.path.join('..', f'{folder}-SpeckleNoise') #Change here
    ...
    # save new DataFrame
    aug_df.to_csv(os.path.join('..', 'annotations', f'trainExtra50From5ORMore-SpeckleNoise.csv'))
