...
seq = iaa.Sequential([
    iaa.Resize({"height": 1333, "width": 1333})], # Resize images to what fasterrcnn accepts Change here
...
def save_augmentations(images: list, bbs: list, df: pd.DataFrame, filename: str, folder: str, resize: bool = False,
    ...
    for [i, img_a], bb_a in zip(enumerate(images), bbs):
        ...
        aug_img_name = name[0]+'-Resized.jpg' #change here
...
if __name__ == '__main__':
    ...
    output_folder = os.path.join('..', f'{folder}_Resize') #Change here
    ...
    # save new DataFrame
    aug_df.to_csv(os.path.join('..', 'annotations', f'annotationsFor225SelectedTrainImages-Resized.csv'))
