...
seq = iaa.Sequential([
    iaa.ShearX((-30, 30), 1)], # Change here
...
def save_augmentations(images: list, bbs: list, df: pd.DataFrame, filename: str, folder: str, resize: bool = False,
    ...
    for [i, img_a], bb_a in zip(enumerate(images), bbs):
        ...
        aug_img_name = name[0]+'-ShearX.jpg' #change here
...
if __name__ == '__main__':
    ...
    output_folder = os.path.join('..', f'{folder}-ShearX') #Change here
    ...
    # save new DataFrame
    aug_df.to_csv(os.path.join('..', 'annotations', f'annotationsFor225SelectedTrainImages-ShearX.csv'))
