...
seq = iaa.Sequential([
    iaa.Add((-50, 50), per_channel=0.0)], # Change here
...
def save_augmentations(images: list, bbs: list, df: pd.DataFrame, filename: str, folder: str, resize: bool = False,
    ...
    for [i, img_a], bb_a in zip(enumerate(images), bbs):
        ...
        aug_img_name = name[0]+'-ArithmeticAdd.jpg' #change here
...
if __name__ == '__main__':
    ...
    output_folder = os.path.join('..', f'{folder}-ArithmeticAdd') #Change here
    ...
    # save new DataFrame
    aug_df.to_csv(os.path.join('..', 'annotations', f'annotationsFor50SelectedTestImages-ArithmeticAdd.csv'))
