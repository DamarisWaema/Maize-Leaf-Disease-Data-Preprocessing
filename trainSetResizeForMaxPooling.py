...
seq = iaa.Sequential([
    iaa.Resize({"height": 666, "width": 666})], # Change here
...
def save_augmentations(images: list, bbs: list, df: pd.DataFrame, filename: str, folder: str, resize: bool = False,
    ...
    for [i, img_a], bb_a in zip(enumerate(images), bbs):
        # define new name
        name=list({filename})[0].split('-MaxPooling.jpg')
        #aug_img_name = f'{filename}_{i}.jpg'
        aug_img_name = name[0]+'-MaxPooling.jpg' #change here
...
if __name__ == '__main__':
    # specify folder
    folder = 'OriginalFinal34TestImages-MaxPooling1'
    ...
    output_folder = os.path.join('..', f'{folder}-MaxPooling2') #Change here
    ...
    df = pd.read_csv(os.path.join('..', 'annotations', f'aTest-Resized-Maxpooling1-edited.csv'))
    ...
    # save new DataFrame
    aug_df.to_csv(os.path.join('..', 'annotations', f'aTest-Resized-Maxpooling2.csv'))
